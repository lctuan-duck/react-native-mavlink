...
- **Features**:
  ...
  - ACK handling (MAV_RESULT_* mappings)
...
1. Connect → Wait for HEARTBEAT → Verify systemId
2. Arm → Verify armed status change
3. Takeoff → Monitor altitude increase
4. Goto → Check position updates toward target
5. Land → Verify descent and disarm
